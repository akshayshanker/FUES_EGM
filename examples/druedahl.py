...
class ConsumerProblem:
    ...
    Pi : array_like(float)
            A 1D NumPy array giving joint probabiltiy of the transitory and 
            permanent i.i.d shocks.
    z_vals : 2D array_like(float)
            2D array of values of permanent (0) and transitory shocks (1)
    ...
    def __init__(self,
        ...
        self.X_hat = cartesian([np.arange(len(self.asset_grid_H)),
                                np.arange(len(self.asset_grid_A))])
        ...
        def du_c(c,h):
            ...
            return (1-alpha)*(c**(gamma_c*(alpha-1) - alpha))*(h**(alpha*(1-gamma_c)))

        @njit(error_model="numpy") 
        def du_h(c,h):
            "Derivative of utility function wrt to housing"
            return max(1e-100,alpha*(c**((1-alpha)*(1-gamma_c))))*max(1e-100,(h**(alpha*(1-gamma_c)-1)))

        @njit(error_model="numpy") 
        def du_c_inv(uc, h):
            "Inverse of MUC holding current period housing fixed"

            exp_h = alpha*(gamma_c-1)/(gamma_c*(alpha-1)-alpha)
            exp_uc = 1/(gamma_c*(alpha-1)-alpha)
        ...
        def u(c,h):
            "The utility function"
            #if c>0 and h>0:
            return ((c**((1-gamma_c)*(1-alpha)))*(h**(alpha*(1-gamma_c)))-1)/(1-gamma_c)
            #else:
            #	return -np.inf


        self.u, self.du_c, self.term_u, self.term_du\
            = u, du_c, term_u, term_du
...
def Operator_Factory(cp):
    ...
    @njit
    def roots(f, a, l, h_prime, z, Lambda_A_next, Lambda_H_next, t, eps=1e-3):

        sols_array = np.zeros(EGM_N)
        i = 0
        while True:
            x1, x2 = rootsearch(f, a, l, eps, h_prime, z,
                                Lambda_A_next, Lambda_H_next, t)
            if np.isnan(x1) == False:
                a = x2
                root = brentq(f, x1, x2,
                              args=(h_prime, z, Lambda_A_next, Lambda_H_next, t),
                              xtol=1e-12)
                if root is not None:
                    sols_array[i] = root[0]
    ...
    @njit
    def obj_noadj(a_prime, w, h, z, i_z, VF_next, R, R_H, chi, t):
        ...
        if w - a_prime[0] > 0 and a_prime[0] > b:
            ...
            # else:
            Ev_prime = eval_linear(UGgrid_all, VF_next[i_z], point, xto.LINEAR)
    ...
    @njit
    def obj_noadj_scalr(a_prime, w, h, z, i_z, VF_next, R, R_H, chi, t):

        return obj_noadj(np.array([a_prime]), w, h, z, i_z, VF_next, R, R_H, chi, t)
        # objective function to be *maximised* for non-adjusters

    @njit
    def obj_adj(x_prime, a, h, z, i_z, VF_next, R, R_H, t):
        ...
        if w_2 > 0 and h_prime >= b:
            args_nadj_2 = (w_2, h_prime, z, i_z, VF_next, R, R_H, chi, t)
            ...
            #obj_x_prime_noadj_zero = obj_noadj_scalr(b, w_2, h_prime, z, i_z, VF_next,R, R_H,chi,t)
    ...
    @njit
    def bellman_operator(t, VF_next):
        """
        The approximate Bellman operator, which computes and returns the
        updated value function TV (or the VF_next-greedy policy c if
        return_policy is True).

        Parameters
        ----------
        VF_next : array_like(float)
        ...
        return_policy : bool, optional(default=False)
                Indicates whether to return the greed policy given VF_next or the
                updated value function TV.  Default is TV.

        Returns
        -------
        array_like(float)
                Returns either the greed policy given VF_next or the updated value
                function TV.

        """

        EV = np.empty(VF_next.shape)
        new_h_prime = np.empty(VF_next.shape)  # next period capital
        new_c = np.empty(VF_next.shape)
        new_a_prime = np.empty(VF_next.shape)  # lisure
        new_V_adj = np.empty(VF_next.shape)
        new_V_noadj = np.empty(VF_next.shape)
        new_a_prime_adj = np.empty(VF_next.shape)
        new_h_prime_adj = np.empty(VF_next.shape)
        ...
        for state in range(len(X_all)):
            ...
            args_adj = (a, h, z, i_z, VF_next, R, R_H, t)
            ...
            args_nadj = (w_noadj, h * (1 - delta), z, i_z, VF_next, R, R_H, 0, t)
            ...
            v_adj = obj_adj(h_prime_adj_star,
                            a, h, z, i_z, VF_next, R, R_H, t)

            v_nadj = obj_noadj_scalr(a_prime_nadj_star[0],
                                     w_noadj, h * (1 - delta), z, i_z, VF_next, R, R_H, 0, t)
            ...
            #obj_at_a_prime_adj_star_zero  = obj_noadj_scalr(b, w_adj_1, h_prime_adj_star, z, i_z, VF_next,R, R_H,chi,t)
            ...
            new_h_prime[i_z, i_a, i_h], new_a_prime[i_z, i_a, i_h],\
                EV[i_z, i_a, i_h] = h_prime, a_prime, v
            new_c[i_z, i_a, i_h] \
        ...
        return new_a_prime, new_h_prime, EV, new_c, \
            new_a_prime_adj, new_h_prime_adj

    @njit
    def root_A_liq_euler_inv(a_prime, h_prime, Lambda_A_next):
        ...
        Lambda_A_next_val = eval_linear(UGgrid_all,
                                     Lambda_A_next,
                                     point, xto.LINEAR)

        c = du_c_inv(Lambda_A_next_val,h_prime)
        egm_a = c + a_prime  # - y_func(t,z)

        return egm_a, c

    @njit
    def housing_euler_resid_(a_prime, h_prime, Lambda_A_next, Lambda_H_next):
        ...
        egm_a, c = root_A_liq_euler_inv(a_prime, h_prime,
                                        Lambda_A_next)

        point = np.array([a_prime, h_prime])
        Lambda_H_next_val = eval_linear(UGgrid_all, Lambda_H_next,
                                     point, xto.LINEAR)

        return du_c(c,h_prime) * (1 + tau) - Lambda_H_next_val - du_h(c,h_prime)

    @njit
    def root_H_UPRIME_func(h_prime, Lambda_A_next, Lambda_H_next):
        ...
        a_prime_points = roots(housing_euler_resid_,
                               asset_grid_A[0],
                               asset_grid_A[-1],
                               h_prime, Lambda_A_next, Lambda_H_next)
        ...
        for j in range(len(a_prime_points)):
            ...
            if a_prime_points[j] > 0:
                ...
                egm_a, c = root_A_liq_euler_inv(a_prime_points[j], h_prime,
                                        Lambda_A_next)

                #c = du_c_inv((Lambda_H_next_val
                #              + du_h(h_prime)) / (1 + tau),h_prime)
        ...
        if len(np.where(a_prime_points == b)[0]) == 0:
            ...
            egm_a, c = root_A_liq_euler_inv(b, h_prime,
                                        Lambda_A_next)

            c_at_amin = du_c_inv((Lambda_H_next_val + du_h(c,h_prime)) / (1 + tau),h_prime)

            # if du_c(c_at_amin)>= Lambda_A_next_val:
            a_prime_points[-1] = b
    ...
    @njit
    def eval_no_adj_pol_egm_(Lambda_A_next, Lambda_H_next, VF_next, t):
        """
        ...
        for index_a_prime in range(len(asset_grid_A)):
            for index_h_today in range(len(asset_grid_H)):
                ...
                egm_a, c = root_A_liq_euler_inv(a_prime, h_prime,
                                                ...
                                                Lambda_H_next)
                #c = max(1e-100, c)
                endog_grid_unrefined[index_a_prime, index_h_today] = egm_a
                c_unrefined[index_a_prime, index_h_today] = c
                ...
                vf_unrefined[index_a_prime, index_h_today]\
    ...
    def refine_no_adj(endog_grid_unrefined,
        ...
        for index_h_today in range(len(asset_grid_H)):
            vf_unrefined_points = vf_unrefined[ :, index_h_today]
            endog_grid_unrefined_points\
                = endog_grid_unrefined[ :, index_h_today]
            c_unrefined_points = c_unrefined[ :, index_h_today]
            ...
            new_a_prime_refined[ :, index_h_today]\
                = interp_as(e_grid_cean, a_prime_clean, asset_grid_A)
            new_c_refined[ :, index_h_today]\
                = interp_as(e_grid_cean, c_clean, asset_grid_A)
            new_v_refined[ :, index_h_today]\
                = interp_as(e_grid_cean, vf_clean, asset_grid_A)

            new_a_prime_refined[ :, index_h_today][np.where(
                new_a_prime_refined[ :, index_h_today] < b)] = b

            for i_a in range(len(asset_grid_A)):
                if asset_grid_A[i_a] < a_under_bar:
                    new_a_prime_refined[ i_a, index_h_today] = b
                    point = np.array(
                    ...
                    v_at_amin = u(c,
                                    ...
                                    0) + beta * eval_linear(UGgrid_all,
                                                            ...
                                                            point,
                                                            xto.LINEAR)
                    new_v_refined[ i_a, index_h_today] = v_at_amin
                    new_c_refined[i_a, index_h_today] = c

        return new_a_prime_refined, new_c_refined, new_v_refined,\
            e_grid_cean, vf_clean, c_clean, a_prime_clean

    @njit
    def eval_adj_pol_egm_(Lambda_A_next, Lambda_H_next, VF_next, t):
        ...
        for index_h_prime in range(len(asset_grid_H)):
            ...
            endog_grid_unrefined[index_h_prime, :] = e_grid_points
            a_prime_unrefined[ index_h_prime, :] = a_primes
            ...
            for i in range(len(a_primes)):
                if a_primes[i] > 0:
                    ...
                    v_prime = beta * eval_linear(UGgrid_all, VF_next,
                                                    point, xto.LINEAR)

                    vf_unrefined[ index_h_prime, i] = u(
                        c_val, h_prime, chi) + v_prime
                    h_prime_unrefined[index_h_prime, i] = h_prime
    ...
    def refine_adj(endog_grid_unrefined,
        ...
        a_prime_unrefined_ur = np.ravel(a_prime_unrefined)

        vf_unrefined_points = np.ravel(vf_unrefined)[
            np.where(a_prime_unrefined_ur > 0)]
        hprime_unrefined_points = np.ravel(h_prime_unrefined)[
            np.where(a_prime_unrefined_ur > 0)]
        aprime_unrefined_points = np.ravel(a_prime_unrefined)[
            np.where(a_prime_unrefined_ur > 0)]
        egrid_unref_points = np.ravel(endog_grid_unrefined)[
            np.where(a_prime_unrefined_ur > 0)]
    ...
    def coleman_operator(t, V_prime,
        ...
        Note: V_prime,Lambda_A_next, Lambda_H_next assumed
        ...
        curr_VF = np.empty(V_prime.shape)
        curr_U_C = np.empty(V_prime.shape)
        curr_U_H = np.empty(V_prime.shape)

        endog_grid_unrefined_noadj, vf_unrefined_noadj, c_unrefined_noadj\
            = eval_no_adj_pol_egm_(Lambda_A_next, Lambda_H_next, V_prime, t)
        ...
        endog_grid_unrefined_adj, vf_unrefined_adj, a_prime_unrefined_adj,\
            ...
            = eval_adj_pol_egm_(Lambda_A_next, Lambda_H_next, V_prime, t)
        ...
        for state in range(len(X_all)):
            ...
            # for adjusters
            wealth = R * a/z[0] + R_H * h * (1 - delta) + z[1]

            a_adj_val = interp_as(asset_grid_WE, a_prime_adj,
                                  np.array([wealth]))[0]
            h_adj_val = interp_as(asset_grid_WE, h_prime_adj,
                                  np.array([wealth]))[0]
            ...
            curr_VF[i_z, i_a, i_h] = d_adj * v_adj_val\
                + (1 - d_adj) * v_nadj_val

            curr_U_C[i_z, i_a, i_h] = beta * R * d_adj * du_c(c_adj_val,h_adj_val) \
                + (1 - d_adj) * beta * R * du_c(c_nadj_val,h_prime_nadj_val)

            # margina value of housing today if not adjusting from POV of
            # t-1

            Phi_t = du_h(c_nadj_val, h_prime_nadj_val) \
                + eval_linear(UGgrid_all,
                              Lambda_H_next[i_z],
                              point_nadj, xto.LINEAR)

            curr_U_H[i_z, i_a, i_h] = beta * R_H * (1 - delta)\
                * (d_adj * du_c(c_adj_val, h_adj_val)
                   + (1 - d_adj) * Phi_t)

        return new_a_prime, new_h_prime, curr_VF, curr_U_C,\
            curr_U_H, new_c, \
            new_h_prime_adj, new_a_prime_adj, new_c_adj,\
            adj_unrefined_grids, adj_refined_grids, adj_nu_clean

    def condition_V(curr_VF, curr_U_C, curr_U_H):
        """ Condition the t+1 continuation vaue on
        time t information"""

        EV = np.zeros(X_hat.shape)
        ELambda_A = np.zeros(X_hat.shape)
        ELambda_H = np.zeros(X_hat.shape)
        ...
        for state in range(len(X_all)):
            ...
            EV[i_a, i_h] = np.dot(Pi, curr_VF[:, i_a, i_h])
            ELambda_A[i_a, i_h] = np.dot(
                Pi, curr_U_C[:, i_a, i_h])
            ELambda_H[i_a, i_h] = np.dot(
                Pi, curr_U_H[:, i_a, i_h])

        return EV, ELambda_A, ELambda_H
